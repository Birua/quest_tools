    ...
    if request.method == "POST":
        ...
        if request.form.get("braille_txt"):
            try:
                ...
                if zapros.replace(" ", "").isnumeric():
                    ...
                    otvet += (
                        "<br> <braille>"
                        + braille.translate_text(otvet.split("<br>")[1])
                        + "</braille>"
                    )
                else:
                    if max([ord(_) for _ in zapros]) > 10_000:
                        ...
                        otvet = (
                            "<braille>" + braille.translate_text(zapros) + "</braille>"
                        )
                ...
                if len(zapros.split()) == 1:
                    ...
                    if zapros.isnumeric():
                        ...
                        if len(mendel.elementByNumber(zapros)[1]) > 1:
                            otvet = " ".join(
                                [str(_) for _ in mendel.elementByNumber(zapros)]
                            )
                        else:
                        ...
                        otvet = " ".join(
                            [str(_) for _ in mendel.elementByNameRu(zapros)]
                        )
                        if otvet.strip() == "":
                            # по символу элемента - Ti Na Sb etc.
                            otvet += " ".join(
                                [str(_) for _ in mendel.elementBySymbol(zapros)]
                            )
                else:
                    ...
                    if zapros.replace(" ", "").isnumeric():
                        ...
                        if otvet.strip() == "":
                            ...
                            for element_name in zapros.split():
                                otvet += (
                                    str(mendel.elementBySymbol(element_name)[0]) + " "
                                )
        ...
        if request.form.get("alpha_txt"):
            try:
                ...
                alphabet_ua = "0абвгґдеєжзиіїйклмнопрстуфхцчшщьюя"
                ...
                if zapros.replace(" ", "").isnumeric():
                    ...
                    for letter_number in zapros.split():
                        ...
                        otvet_ru += (
                            alphabet_ua[letter_number]
                            if letter_number < len(alphabet_ua)
                            else ""
                        )
                        otvet_en += (
                            alphabet_en[letter_number]
                            if letter_number < len(alphabet_en)
                            else ""
                        )
                    ...
                    for letter in zapros.lower():
                        letter_number = ""
                        # выдавать номер только для букв, которые есть в alphabet_ua
                        letter_number = (
                            str(alphabet_ua.index(letter))
                            if letter in alphabet_ua
                            else ""
                        )
                        if letter_number == "":
                            # буквы не было в русском, ищем в английском
                            letter_number = (
                                str(alphabet_en.index(letter))
                ...
                zapros = ""
                otvet = "Error: сбой блока Alpha:" + repr(ex)
                add_to_output(zapros, otvet)
            return redirect("/")
        ...
        if request.form.get("anagrams_txt"):
            ...
            try:
                ...
                zapros = ""
                otvet = "Error: сбой блока Anagrams" + repr(ex)
                add_to_output(zapros, otvet)
            return redirect("/")
            ...
            try:
                ...
                if len(zapros.split()) == 1:
                    ...
                    otvet = (
                        zapros.strip()
                        + " = "
                        + olymp.getSociationsForOne(zapros.strip())
                    )
        ...
        if request.form.get("caesar_txt"):
            ...
            try:
                ...
                zapros = ""
                otvet = "Error: сбой блока Caesar" + repr(ex)
                add_to_output(zapros, otvet)
            return redirect("/")
        # --------------------------------------------------
        # Семафорная азбука
        ...
        if request.form.get("semaphore_txt"):
            ...
            try:
                ...
                if (
                    len(set(zapros).intersection("абвгдеёжзийклмнопрстуфхцчшщъыьэюя"))
                    == 0
                ):
                    # English
                    otvet = (
                        "Необходимо вводить коды левой руки и правой согласно рисунку."
                    )
                else:
                    # Russian
        ...
        if request.form.get("mask_txt"):
            ...
            try:
                ...
                # alphabet_ua = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя'
                mask = MaskSearch()
                if len(set(zapros).intersection(alphabet_en)) == 0:
                    # Значит UA
                    otvet = mask.mask_search_ua_SQL(zapros.strip())
                else:
                    # английский
                ...
                zapros = ""
                otvet = "Error: сбой блока Mask:" + repr(ex)
                add_to_output(zapros, otvet)
            return redirect("/")

    return render_template(
        "quest_tools.html",
        output_window=Markup(session.get("output_window", "")),
        current_version="0.19 UA",
    )
